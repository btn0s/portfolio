...
Strella is my dream project—a node-based IDE where designers and engineers speak the same language. Inspired by Unreal Engine's <Link href="https://dev.epicgames.com/documentation/en-us/unreal-engine/blueprints-visual-scripting-in-unreal-engine" target="_blank" rel="noopener noreferrer">Blueprints</Link>, it replaces the fragmented workflow of Figma-to-code with a unified canvas where visual components connect directly to functional logic, eliminating the translation gap.
...
There's a super simple landing page for the project [here](https://strella.dev), feel free to check it out and even drop your email if you're interested in being notified when I have something to show.
