...
Strella is my dream project—a node-based IDE where designers and engineers speak the same language. Inspired by Unreal Engine's [Blueprints](https://en.wikipedia.org/wiki/Blueprints_(Unreal_Engine)), it replaces the fragmented workflow of Figma-to-code with a unified canvas where visual components connect directly to functional logic, eliminating the translation gap.
